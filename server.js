...
// ================================================================
// server.js に追加するパッチ
// 既存の app.listen(...) の直前にこのブロックを貼り付けてください
// ================================================================

// ════════════════════════════════════════════════════════
//  POST /api/obfuscate  — カスタムVM難読化
//  vm_obfuscator_fixed.lua を呼び出して難読化する
// ════════════════════════════════════════════════════════
app.post('/api/obfuscate', async (req, res) => {
  const { code, seed } = req.body;
  if (!code) return res.json({ success: false, error: 'コードが提供されていません' });
  ...
  // Luaインタープリタの確認
  const luaCmd = checkLuaAvailable();
  if (!luaCmd) {
    return res.json({ success: false, error: 'Luaインタープリタが見つかりません' });
  }

  // vm_obfuscator_fixed.lua のパス
  const obfuscatorPath = path.join(__dirname, 'vm_obfuscator_fixed.lua');
  if (!fs.existsSync(obfuscatorPath)) {
    return res.json({
      success: false,
      error: 'vm_obfuscator_fixed.lua が見つかりません。ルートディレクトリに配置してください'
    });
  }

  const id      = makeTempId();
  const inFile  = path.join(tempDir, `obf_in_${id}.lua`);
  const outFile = path.join(tempDir, `obf_out_${id}.lua`);
  ...
  try {
    ...
    // オプション組み立て
    const seedArg = seed ? `--seed ${parseInt(seed) || ''}` : '';
    const cmd = `${luaCmd} "${obfuscatorPath}" "${inFile}" --out "${outFile}" ${seedArg}`;

    const result = await new Promise((resolve) => {
      exec(cmd, { timeout: 30000, maxBuffer: 8 * 1024 * 1024 }, (err, stdout, stderr) => {
        resolve({ err, stdout, stderr });
      });
    });

    // stdout に "OK:<outfile>" が含まれているか確認
    const isOk = result.stdout && result.stdout.startsWith('OK:');

    if (!isOk || result.err) {
      const errMsg = (result.stderr || '') + (result.err ? result.err.message : '');
      console.error('[obfuscate] error:', errMsg);
      return res.json({
        success: false,
        error: errMsg || '難読化に失敗しました',
    ...
    if (!fs.existsSync(outFile)) {
      return res.json({ success: false, error: '出力ファイルが生成されませんでした' });
    }
    ...
    return res.json({
      ...
      info:    result.stderr || null,
    });

  } catch (e) {
    console.error('[obfuscate] exception:', e);
